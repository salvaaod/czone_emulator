...
const mapLines=Object.entries(s.mappings).map(([circuit,loads])=>`${circuit}: `+loads.join(', '));const cfg=s.mapping_config_status||{};const file=cfg.config_file||{};const fileText=file.exists?`Configuration file: ${file.filename||file.configured_filename} | Internal name: ${file.internal_name||file.metadata_error||'unknown'} | Size: ${file.size_label||file.size_bytes||'unknown'}`:'Configuration file: not found';const mapping=document.getElementById('mapping');mapping.className='mapping-line';mapping.innerText=`Mapping source: ${cfg.message||'Unknown'} | ${fileText} | Circuit load mappings: ${mapLines.join(' | ')}`;
